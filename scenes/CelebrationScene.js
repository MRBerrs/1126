class CelebrationScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.input.on("pointerdown", (pointer) => {
      ...
      this.tweens.add({ targets: c, scale: 2.5, alpha: 0, duration: 400, onComplete: () => c.destroy() });
    });
  }
  ...
  spawnConfetti() {
    ...
    particles.createEmitter({
      x: { min: 0, max: width }, y: -10, lifespan: 3000,
      speedY: { min: 100, max: 300 }, speedX: { min: -100, max: 100 },
      scale: { start: 1, end: 0.5 }, quantity: 30, blendMode: "NORMAL",
  ...
  tapConfetti(x, y) {
    const particles = this.add.particles("dot");
    particles.createEmitter({
      x, y, lifespan: 1200, speed: { min: 100, max: 250 },
      angle: { min: 0, max: 360 }, scale: { start: 1, end: 0.5 },
      quantity: 20, tint: [0xff5e5e, 0xffd86b, 0x7fe7a4, 0x79c2ff, 0xd89bff]
    });
